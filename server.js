...
db.connect((err) => {
    if (err) {
        console.error('ERROR IN ESTABLISHING CONNECTION WITH MySQL DATABASE...', err);
        process.exit(1);
    }
...
app.post('/signup', async (req, res) => {
    ...
    db.query(checkQuery, [username], async (err, results) => {
        ...
        if (results.length > 0) {
            res.status(400).json({ error: 'SOMEONE IS HAVING SAME CHOICE AS YOURS. PLEASE CHOOSE DIFFERENT USERNAME' });
            return;
        }
        ...
        // |INSERTING VALUES| //
        const insertQuery = 'INSERT INTO Users (username, password, email, phone, qrCode) VALUES (?, ?, ?, ?, ?)';
...
app.post('/forgot-password', (req, res) => {
    ...
    db.query(query, [username, email], (err, results) => {
        ...
        if (results.length === 0) {
            res.status(401).json({ error: 'OOPS! INVALID USERNAME OR EMAIL. PLEASE TRY AGAIN' });
            return;
        }
        ...
        transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
                ...
                res.status(500).json({ error: 'ERROR IN SENDING EMAIL. PLEASE TRY AGAIN' });
                return;
            }
            res.json({ success: true, redirect: 'log-in.html' });
        });
